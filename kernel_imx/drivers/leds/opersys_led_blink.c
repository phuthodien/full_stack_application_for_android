...
#define GPIO_BASE_ADDR		0x20A8000
#define GPIO_DATA_REG		0x0
#define GPIO_DIR_REG			0x4
#define GPIO102_DIR_OUT		(0x1 << 0x6)
#define GPIO102_LED_ON		(0x1 << 0x6)
#define GPIO102_LED_OFF		0x00
...
static ssize_t dev_write(struct file *fp, const char __user *buf, size_t len, loff_t *off)
{
	...
	if (msg[0] == '1')
	{
		iowrite32(GPIO102_LED_ON, gpio_base + GPIO_DATA_REG);
		//gpio_set_value(gpio_led, 1);
	}
	else if (msg[0] == '0')
	{
		iowrite32(GPIO102_LED_OFF, gpio_base + GPIO_DATA_REG);
...
static int __init udoo_init(void)
{
	...
	iowrite32(GPIO102_DIR_OUT, gpio_base + GPIO_DIR_REG);

	ret = alloc_chrdev_region(&dev_num, 0, 1, "my_misc_device");
	if (ret < 0)
	{
	...
	dev = device_create(cls, NULL, dev_num, NULL, "my_misc_device");
	if (IS_ERR(dev))
	{
